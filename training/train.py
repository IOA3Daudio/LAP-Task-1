...
def train(epoch,pp):
    ...
    for batch_idx, (x_data,fb_data,e_data,onset_data,l_data) in enumerate(dataloader):
        ...
        loss = lossF(out,fb_data,e_data,l_data,epoch,pp)
...
class My_loss(nn.Module):
    ...
    def forward(self, hrir_pred, ori_fb, ori_p, labels, epoch, pp):
        ...
        pred_mag = torch.abs(torch.fft.fft(hrir_pred,512,dim=2))
        ...
        # 总损失
        loss_cls = inter_distances
        # print(loss_cls)


        loss = loss_p + loss_fb + loss_cls*10
...
for pp in range(126):
    ...
    train_dataset = myDataset(train_hrirs_fake,train_target_fb,train_target_e,train_onseth_all,train_labels_all)
